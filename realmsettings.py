...
class DefaultFactory:
    ...
    @staticmethod
    def realm_entry(name, /, strings=None, *, hidden=False, **kwds):
...
class ItemAccessBase(ABC):
    ...
    @abstractmethod
    def popitem(self, /):
        raise KeyError(f'{self.__class__} have no items')
...
class _ProxyDescriptorBase(ABC):
    ...
    def __init__(self, obj: Union[Loader, Dumper], *args, **kwargs) -> None:
...
class DefaultLoader(_ProxyDescriptorBase, Loader):
    ...
    def __set__(self, obj, value):
        if (
            isinstance(value, Loader)
            or hasattr(value, 'load')
            and callable(value.load)
        ):
...
class DefaultDumper(_ProxyDescriptorBase, Dumper):
    ...
    def __set__(self, obj, value):
        if (
            isinstance(value, Dumper)
            or hasattr(value, 'dump')
            and callable(value.dump)
        ):
            super().__set__(obj, value)
        else:
...
class CoreSettings(SettingsData, BaseSettings, Saveable):
    ...
    def load(
        ...
        try:
            ...
            print(f"Can't load '{self.filename}': {ex}", file=self.errfile)
        else:
    ...
    def save(
        ...
        try:
            ...
            print(f"Can't save '{self.filename}': {ex}", file=self.errfile)
        else:
            return True
        return False
